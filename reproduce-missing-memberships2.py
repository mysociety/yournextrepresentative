...
try:
    ...
    birth_date = "1970-04-{0:02d}".format(randint(1, 30))
    ...
    print "Now deleting all their memberships:"
    for m in person_data['result']['memberships']:
        print "Deleting membership:", m['person_id'], "member of", m['organization_id']
        api.memberships(m['id']).delete()

    print "Add back the membership back"
    ...
    person_data = api.persons(person_id).get()
    memberships = person_data['result'].get('memberships', [])
    print "The number of memberships is:", len(memberships)
    if len(memberships) == 0:
        print "###### Got no memberships back!"
