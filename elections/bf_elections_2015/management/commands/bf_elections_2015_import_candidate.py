...
def get_post_data(api, election_id, province=None):
    ynr_election_data = settings.ELECTIONS[election_id]
    ynr_election_data['id'] = election_id
    mapit_key = ( ynr_election_data['mapit_types'][0], ynr_election_data['mapit_generation'])
    if province:
        ...
        post_id = ynr_election_data['for_post_role']
...
def windows_1252_csv_reader(csvfile):
    for encoded_row in csv.reader(csvfile):
        yield [c.decode('windows-1252') for c in encoded_row]
    ...
    def handle(self, username=None, **options):
        ...
        for election_id, filename in election_data.items():
            ...
            with open(csv_filename) as f:
                ...
                initial = True
                for candidate in windows_1252_csv_reader(f):
                    # skip header line
                    if initial:
                        initial = False
                        continue
                    ...
                    # debug
                    #tmp = '%s %s %s (%s) - %s (%s)' % ( id, first_name, last_name, party, region, post )
                    #print tmp
